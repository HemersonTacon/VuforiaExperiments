...
public class SampleAppMenu
{
    ...
    // percentagem da tela que o menu ocupa quando esta sendo mostrado
    private static float SETTINGS_MENU_SCREEN_PERCENTAGE = .80f;
    // percentagem minima da tela para comecar a mostrar o menu
    ...
    public boolean processEvent(MotionEvent event)
    {
        ...
        // chama a callback para lidar com os eventos de toque na tela, retorna true se o evento foi "lidado"
        result = mGestureDetector.onTouchEvent(event);
        
        if (event.getAction() == MotionEvent.ACTION_UP && !result)
        {
            // se tiver levantado o dedo da tela, coloca o swiping como falso
            setSwipingMenu(false);
            
            // Hides the menu if it is not docked when releasing
            // esconde ou mostra o menu de acordo com o comportamento estabelecido pelas porcentagens de tela relacionadas ao menu
            if (!isMenuDisplaying()
